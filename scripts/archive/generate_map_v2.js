...
async function generateWorldMapV2() {
    const envContent = fs.readFileSync('.env', 'utf8');
    const apiKey = envContent.match(/GEMINI_API_KEY=(.*)/)[1].trim();
    const model = 'gemini-3.1-flash-image-preview';
    const outputPath = 'assets/world-map-epic-v2.png';
...
The map shows a vast continental landmass filled with intricate miniature geography: dense networks of tiny river veins, jagged clusters of miniature mountain peaks, and rich textured forests. The scale is epic and world-sized, similar to the Forgotten Realms.

NEW FEATURE: A network of roads that follow the natural terrain and intersect logically across rivers.
...
- MAP GOES FULLY TO THE EDGE, NO BORDERS.`;
    ...
    console.log("Generating EPIC WORLD MAP V2 (with Integrated Roads)...");
    ...
    try {
        ...
        if (response.data.candidates && response.data.candidates[0].content) {
            ...
            if (imagePart) {
                ...
                console.log(`Success! World Map V2 saved to ${outputPath}`);
            } else {
